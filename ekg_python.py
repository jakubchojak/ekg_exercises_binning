...
def binning(time_lineup, RR_lineup, binning_interval):
    ...
    for i in range(len(time_lineup)):
        ...
        if time_lineup[i] >= start_time + binning_interval:
            binning_effect.append((time_sum / h_m_samples, sum_RR / h_m_samples))
            h_m_samples = 0
...
def set_range(tup_gen, min_minutes, max_minutes, time_lineup, RR_lineup, binning_interval = 0.0):
    i = 0
    ...
    if binning_interval > 0.0:
        return binning(time_lineup, RR_lineup, binning_interval)
...
next_show = set_range(tup_gen, min_v, max_v, time_lineup, RR_lineup, bin_interval)

time_next = []
RR_next = []

for i in range(len(next_show)):
    time_next.append(next_show[i][0])
    RR_next.append(next_show[i][1])
